...
module Provider
  ...
  class VSphere
    ...
    def create_vm(opts = {})
      opts = opts.symbolize_keys.reverse_merge(VSphereConfig.create_vm_defaults || {}).with_indifferent_access
      p VSphereConfig.create_vm_defaults
      opts = opts.merge(template_path: compute.image) if compute.image

      VSphere.connect.with do |vs|
        dest_folder = opts[:dest_folder]
        vm_name = opts[:name] || 'lm_' + SecureRandom.hex(8)
        exception_cb = lambda do |p1|
          LabManager.logger.warn(
            "Failed attempt to create virtual machine:  template_name: #{opts[:template_path]}, vm_name: #{vm_name}"
          )
        end
        Retryable.retryable(
          tries: 3,
